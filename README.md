...
| --- | --- | --- |
| **Fear & Greed** (`/vibe`) | *The Fear & Greed Gazette*, a black-and-white newspaper. The front page is the PreStocks market index on a gauge, the stories moving it, a board of every company (today, 7-day, 30-day) and a crypto benchmark. Each company has a section: its gauge, factor breakdown, score over price, and every headline with the words that scored it and its source. | Google News RSS (headline-filtered, spam-filtered, 45-day backfill), on-chain candles, PreStocks mark vs token price |
